...
import zio.nio.core.{Buffer, ByteBuffer, CharBuffer}

import scala.collection.JavaConverters._

final class Charset private (val javaCharset: j.Charset)
    extends Ordered[Charset] {
  ...
  def decode(byteBuffer: ByteBuffer): UIO[CharBuffer] =
    byteBuffer.withJavaBuffer(jBuf =>
      UIO.effectTotal(Buffer.charFromJava(javaCharset.decode(jBuf)))
    )
  ...
  def encode(charBuffer: CharBuffer): UIO[ByteBuffer] =
    charBuffer.withJavaBuffer(jBuf =>
      UIO.effectTotal(Buffer.byteFromJava(javaCharset.encode(jBuf)))
    )
